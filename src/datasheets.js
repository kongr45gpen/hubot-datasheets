...
module.exports = robot =>
    robot.respond(/datasheet(s?) (.*)/i, function (res) {
        const multiple = res.match[1].toLowerCase().length !== 0;
        const part = res.match[2].toLowerCase().replace(/^\s+|\s+$/g, ""); // trim/strip
        ...
        const respond = function(body) {
            ...
            // For each part that we received
            for (let result of data.results) {
                ...
                if (!datasheets.length) {
                    replies.push(`No datasheets found for ${item.mpn}`);
                    continue;
                }
                ...
                replies.push(`${item.datasheets[0].__class__} for **${item.mpn}**${price_string}: ${item.datasheets[0].url}`)
            ...
            // Send the combination of all the replies
            return res.reply(replies.join("\n"));
        };
